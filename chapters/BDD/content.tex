...
Les paramètres utilisés pour ces simulations (voir tableau \ref{tab:param_lj_horinek}) sont ceux proposés par Horinek et al.\cite{Horinek_rational_2009}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{l c c}
   \hline & \\[-1em]\hline
    ...
    \hline & \\[-1em]\hline
  \end{tabular}
  \caption{Paramètres Lennard-Jones des ions utilisés dans nos simulations.}
...
Pour cela, les auteurs ont considérés la différence entre l'énergie libre de solvatation du composé étudié et celle d'un composé de référence (ici l'ion Cl$^-$). Cette différence, notée $\Delta\Delta G_{\text{solv}}$ s'exprime:
\begin{eqnarray}
