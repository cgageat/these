...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[mark=none, black, very thick, domain=0:10, samples=400] {demouillage(-3.5,1)};
    \end{axis}
  \end{tikzpicture}
    \caption{Représentation du démouillage autour d'une sphère hydrophobe de rayon R.}
...
%   \centering
%   \resizebox{\linewidth}{!}{
%     \fbox{\includegraphics[width=\textwidth]{chapters/bridge/images/diagramme_phase_eau.png}}
%   }
%   \caption{Diagramme de phase de l'eau. [REF \url{http://nte.mines-albi.fr/ThermoIntro/fr/res/ti_01_1.png}]}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \begin{axis}[
            ...
            ylabel= $\Delta \mathrm{g}_{\mathrm{solv}} (\mathrm{kJ.mol}^{-1}.\text{\AA}^{-3}) $,
            xmin = 0, xmax = 1.5,
            ymin = 0, ymax = 0.15,
    ...
    \end{axis}
  \end{tikzpicture}
    \caption{\'Energie libre pour une unité de volume du solvant homogène de densité $\rho_{\mathrm{bulk}}$ dans l'approximation HNC. $\rho_b$ est la densité bulk de l'eau SPC/E à pression et température standards ( 1 $\mathrm{kg.L}^{-1}$ ).}
...
%     %contour
%     %\draw[black, thick, fill=none] (0,0) -- ++(6,0) -- ++(0,6) --++ (-6,0) -- cycle;
%     \draw[black, fill=none] (3,3) circle (5);    
%     %solute
%     \draw[gray, fill=gray] (3,3) circle (1);
%     %solvent
%     \draw[white!70!red, fill=white!40!red] (5,5) circle (0.2);
%     \draw[white!70!red, fill=white!40!red] (1,5) circle (0.2);
%     %fleches
%     %\tikz \draw[<->, white!70!red] (4,4) -- (4.8,4.8);
%     %\tikz \draw[<->, white!70!red] (2,4) -- (1.2,4.8);
%   \end{tikzpicture}
%     \caption{Système à symétrie sphérique. Le soluté, une sphère de Lennard-Jones est représentée en gris. Le site de Lennard-Jones (oxygène) de deux molécules d'eau SPC/E sont représentés en rouge. Ces deux molécules sont identiques dans un système à symétrie sphérique}
%     \label{fig:symetrie_spherique}
% \end{figure}
...
Au contraire des deux premiers termes, la fonctionnelle d'excès n'est pas centrée sur l'origine. Nous ne pouvons donc pas nous placer directement dans des coordonnées polaires. Nous réécrivons donc dans un premier temps, cette partie de la fonctionnelle, sous la forme d'une convolution ce qui nous donne :
...
%            ylabel= $\Delta g$ (kJ/mol/\AA$^3$),
    ...
    \end{axis}
  \end{tikzpicture}
    \caption{Noyaux de convolution utilisés dans l'étude paramétrique permettant la définition du bridge gros grain. En noir le heavyside et en rouge la gaussienne.}
    \label{fig:noyaux}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
    \caption{\'Energie libre d'une unité de volume du solvant homogène de densité $\rho_\mathrm{bulk}$ en fonction du paramètre B ($10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$). $\rho_b$ est la densité bulk de l'eau SPC/E à pression et température standards (1$kg.L^{-1}$).}
    \label{fig:etude_B}
\end{figure}

\subsubsection{\'Exploration des paramètres B et $\sigma$}
Pour chaque valeur de ce paramètre B, nous avons tracé l'énergie libre de solvatation d'une sphère dure grossissante divisée par son volume. Nous avons ensuite ajusté la valeur de $\sigma$ pour obtenir une tension de surface correcte (voir figure \ref{fig:etude_sigma}).
...
\center
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \legend{$\sigma=0.866$, $\sigma=0.900$, $\sigma=0.935$, $\sigma=0.966$, $\sigma=1.000$}
      \end{axis}
    \end{tikzpicture}
	\caption{\'Energie libre de solvatation d'une sphère dure grossissante divisée par sa surface pour différentes valeurs de $\sigma$ (en \AA), largeur de la gaussienne servant à la convolution. B est fixé ici à $15.10^{-8} kJ.mol^{-1}.\text{\AA}^{15}$. La valeur de référence en pointillé est la valeur de la tension de surface de l'eau SPC/E soit 63.3 mJ/m$^2$\cite{vega_surface_2007}.}
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption{ \`Energie libre de solvatation d'une sphère dure grossissante divisée par sa surface calculée dans l'approximation HNC puis avec les différents paramètres du bridge. Ces courbes en rouge, sont comparés à des valeurs de référence (sphères noires) calculées par Monte-Carlo\cite{hummer_information_1996} }
    \label{fig:comparaison_hummer}
\end{figure}
\end{sidewaysfigure}


 On voit que l'approximation HNC, surestime les énergies libres de solvatation. Notre bridge, corrige fortement cet écart, quelques soit le couple de paramètre choisi. Les meilleurs paramètres ici semblent être pour un B autour de $5.10^{-8}\  \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$. 

\boitesimple{ Les \'energies libres de solvatation de sphères dures les plus précises sont obtenus pour un B autour de $5.10^{-8}\  \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$. Chacun des paramètres proposés sont acceptables à ce niveau. }
...
%    Neopentane &       &  0.37 & 84.51 & 17.51 & 13.77 & 11.30 &  9.43 &  8.17 &  7.05 &  \textbf{6.23}  \\
    ...
    \hline & \\[-1em]\hline
  \end{tabular}
  \caption{Valeurs de l'énergie libre de solvatation (en $kj.mol^{-1}$) du méthane unifié et des gaz rares: Argon, Xénon, Krypton, Néon pour les différents paramètres possibles de la fonctionnelle de bridge gros grain. Les valeurs les plus proches de notre référence, la dynamique moléculaire, sont en gras.}
...
\boitesimple{ Les meilleures énergies libres de solvatation de nos molécules modèles sont obtenues avec B=$15.10^{-8}\  \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$  et $\sigma$ = 0.935 \AA }
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption{ Fonction de distribution radiale du solvant autour de sphères dures de rayon 2.0, 4.0, 6.0, 8.0 et 10.0 $\text{\AA}$. Les RDF ont été calculés avec MDFT dans l'approximation HNC puis pour chaque paramètres du bridge. Les références ont été calculées par Monte Carlo\cite{huang_hydrophobic_2002} (pointillés bleu). }
    \label{fig:g_r_HS}
\end{figure}
\end{sidewaysfigure}
...
\boitesimple{ Les meilleurs profils sont obtenus pour les valeurs B = $-5.10^{-8}\ \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$  et $\sigma$ = 1.061 \AA. Les autres paramètres fournissent des résultats légèrement plus éloignés de la référence mais restent cependant acceptables. }
...
%            \legend{MD, HNC, $B=-15$, $B=-10$, $B=-5$, $B=-0$, $B=5$, $B=10$, $B=15$}
  ...
  \end{subfigure}
  \caption{Fonction de distribution radiale du méthane et des gaz rares. Les RDFs ont été calculées avec MDFT dans l'approximation HNC (en noir) et les deux paramètres extrêmes disponibles pour notre bridge (en marron et noir). Les références (en pointillées bleu) ont été calculées par dynamique moléculaire.}
...
\boitesimple{ Les meilleurs profils de solvatation de nos molécules modèles sont obtenus pour les valeurs B = $15.10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$ et $\sigma$ = 0.935 \AA. }
...
F_{\mathrm{b}}[\bar{\rho}(\boldsymbol{r})]=\mathrm{k_B}T(\frac{1}{\rho_0^2} - \frac{\int c(\boldsymbol{r}) d\boldsymbol{r}}{2\rho_0})\int\Delta\bar{\rho}(\boldsymbol{r})^3d\boldsymbol{r}+15.10^{-8}\int\bar{\rho}(\boldsymbol{r})^2\Delta\bar{\rho}(\boldsymbol{r})^4d\boldsymbol{r}
\end{equation} 
}
...
\begin{table}
  ...
  \begin{tabular}{ l c c c c c c c }
    ...
    \hline & \\[-1em]\hline
  \end{tabular}
  \caption{\'Energie libre de solvatation du méthane et des gaz rares en $\mathrm{kJ}.\mathrm{mol}^{-1}$ avec MDFT dans l'approximation HNC et avec le bridge. Les valeurs sont comparées à nos références calculées par Dynamique moléculaire et aux valeurs expérimentales\cite{straatsma_free_1986}.}
...
\centering
 ...
 \begin{subfigure}{.5\textwidth}
  ...
  \end{subfigure}
  \caption{ Fonctions de distribution radiale autour du méthane et des gaz rares. Les RDF ont été calculés avec les versions de MDFT à symétrie sphérique et 3D, dans l'approximation HNC et avec le bridge gros grain. Les références ont été calculées par Dynamique moléculaire.. }
...
Enfin, nous avons étudié la prédiction de profils de solvatation autour de plus grosses molécules et en particulier autour d'une protéine. Cet exemple sera traité et analysé plus en détails dans le chapitre XXXXX.


% IMAGE TEMPORAIRE EN ATTENDANT DE TOUTES LES REFAIRE XXXXXX
% \begin{figure}[H]
%   \centering
%   \resizebox{\linewidth}{!}{
%     \fbox{\includegraphics[width=\textwidth]{chapters/bridge/images/protein_mdft_gromacs_water.png}}
%   }
%   \caption{Zones de forte densité en solvant autour de la protéine 4M7G, calculées par MDFT dans l’approximation HNC en XXX COULEUR et avec le bridge en XXX COULEUR. La référence, en XXX COULEUR, est calculée par Dynamique moléculaire. }
%   \label{fig:g_of_r_protein}
% \end{figure}
% IMAGE TEMPORAIRE EN ATTENDANT DE TOUTES LES REFAIRE XXXXXX
...
Dans ce chapitre nous décrivons le développement d'un nouveau bridge \textbf{simple} et \textbf{rapide}. Ce bridge, basé sur une densité gros grain ajoute de la consistance thermodynamique à nos modèles, en reproduisant une pression et une tension de surface correcte. Il améliore également fortement le calcul de l'énergie libre de solvatation et la prédiction de structure de solvant, sur les petites molécules mais également sur des molécules plus grosses comme des systèmes biologiques. 
}
