...
De plus, Jeanmairet et Al.\cite{jeanmairet_molecular_2013-1} ont montré que le couplage entre la densité et la polarisation de l'eau est négligeable.
...
Pour des sphères de petit rayon, le terme proportionnelle au volume est prépondérant.
...
Pour chaque valeur de ce paramètre B, nous avons tracé l'énergie libre de solvatation d'une sphère dure en fonction de son rayon et divisée par son volume.
...
\center
    \begin{tikzpicture}
      ...
      \end{axis}
    \end{tikzpicture}
	\caption[\'Energie libre de solvatation d'une sphère dure en fonction de son rayon divisée par sa surface en fonction de $\sigma_{gauss}$.]{\'Energie libre de solvatation d'une sphère dure en fonction de son rayon divisée par sa surface en fonction de $\sigma_{gauss}$ (en \AA), largeur de la gaussienne servant à la convolution. B est fixé ici à $15.10^{-8} kJ.mol^{-1}.\text{\AA}^{15}$. La valeur de référence en pointillé est la valeur de la tension de surface de l'eau SPC/E soit 63.3 mJ/m$^2$\cite{vega_surface_2007}.}
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption[\'Energie libre de solvatation d'une sphère dure en fonction de son rayon divisée par sa surface pour différentes valeurs de B.]{ \'Energie libre de solvatation d'une sphère dure en fonction de son rayon divisée par sa surface calculée dans l'approximation HNC puis avec les différents paramètres du bridge. Les calculs ont été effectués pour chaque couple de paramètres B ($10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$) et $\sigma_{gauss}$ (\AA). Ces courbes en rouge, sont comparés à des valeurs de référence (sphères noires) calculées par Monte-Carlo\cite{hummer_information_1996} }
...
\boitesimple{ Les meilleures énergies libres de solvatation de nos molécules modèles sont obtenues avec B=$15.10^{-8}\  \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$  et $\sigma_{gauss}$ = 0.935 \AA }
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption[Fonction de distribution radiale du solvant autour de petites sphères dures.]{ Fonction de distribution radiale du solvant autour de sphères dures de rayon 2.0, 4.0, 6.0, 8.0 et 10.0 $\text{\AA}$. Les RDF ont été calculés avec MDFT dans l'approximation HNC puis pour chaqueLes calculs ont été effectués pour chaque couple de paramètres B ($10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$) et $\sigma_{gauss}$ (\AA) de notre bridge. Les références ont été calculées par Monte-Carlo\cite{huang_hydrophobic_2002} (pointillés bleu). }
    \label{fig:g_r_HS}
\end{figure}
\end{sidewaysfigure}
...
\boitesimple{ Les meilleurs profils sont obtenus pour les valeurs B = $-5.10^{-8}\ \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$  et $\sigma_{gauss}$ = 1.061 \AA. Les autres paramètres fournissent des résultats légèrement plus éloignés de la référence mais restent cependant acceptables. }
...
\boitesimple{ Les meilleurs profils de solvatation de nos molécules modèles sont obtenus pour les valeurs B = $15.10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$ et $\sigma_{gauss}$ = 0.935 \AA. }
