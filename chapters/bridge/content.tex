...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
    \caption[\'Energie libre pour une unité de volume du solvant homogène.]{\'Energie libre pour une unité de volume du solvant homogène de densité $\rho_{\mathrm{bulk}}$ dans l'approximation HNC. $\rho_b$ est la densité bulk de l'eau SPC/E à pression et température standards ( 1 $\mathrm{kg.L}^{-1}$ ).}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
    \caption[\'Energie libre d'une unité de volume du solvant homogène en fonction du paramètre B.]{\'Energie libre d'une unité de volume du solvant homogène de densité $\rho_\mathrm{bulk}$ en fonction du paramètre B ($10^{-8} \mathrm{kJ.mol}^{-1}.\text{\AA}^{15}$).}{ $\rho_b$ est la densité bulk de l'eau SPC/E à pression et température standards (1$kg.L^{-1}$).}
    \label{fig:etude_B}
\end{figure}
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption[\`Energie libre de solvatation d'une sphère dure grossissante divisée par sa surface pour différentes valeurs de B.]{ \`Energie libre de solvatation d'une sphère dure grossissante divisée par sa surface calculée dans l'approximation HNC puis avec les différents paramètres du bridge. Ces courbes en rouge, sont comparés à des valeurs de référence (sphères noires) calculées par Monte-Carlo\cite{hummer_information_1996} }
...
\begin{subfigure}{.24\textwidth}
  ...
  \end{subfigure}
    \caption[Fonction de distribution radiale du solvant autour de petites sphères dures.]{ Fonction de distribution radiale du solvant autour de sphères dures de rayon 2.0, 4.0, 6.0, 8.0 et 10.0 $\text{\AA}$. Les RDF ont été calculés avec MDFT dans l'approximation HNC puis pour chaque paramètres du bridge. Les références ont été calculées par Monte Carlo\cite{huang_hydrophobic_2002} (pointillés bleu). }
    \label{fig:g_r_HS}
\end{figure}
\end{sidewaysfigure}
